...
from typing import Callable, List, Mapping, Sequence, Set
...
def name_match(tidal_track, spotify_track) -> bool:
    def exclusion_rule(
        ...
        tidal_has_pattern = pattern in tidal_track.name.lower() or (
            tidal_track.version is not None and (pattern in tidal_track.version.lower())
        )
...
async def get_tracks_from_spotify_playlist(
    ...
    def _get_tracks_from_spotify_playlist(
        offset: int, spotify_session: spotipy.Spotify, playlist_id: str
    ):
    ...
    return await _fetch_all_from_spotify_in_chunks(
        lambda offset, session=spotify_session: _get_tracks_from_spotify_playlist(
            offset=offset, spotify_session=session, playlist_id=spotify_playlist["id"]
        )
    )
...
async def search_new_tracks_on_tidal(
    ...
    for idx, spotify_track in enumerate(tracks_to_search):
        if search_results[idx]:
            ...
            print(
                color[0]
                + f"Could not find track {spotify_track['id']}: {','.join([a['name'] for a in spotify_track['artists']])} - {spotify_track['name']}"
                + color[1]
            )
...
async def get_playlists_from_spotify(spotify_session: spotipy.Spotify, config):
    ...
    if results["next"]:
        ...
        extra_results = await atqdm.gather(
            *[
                asyncio.to_thread(
                    spotify_session.user_playlists,
